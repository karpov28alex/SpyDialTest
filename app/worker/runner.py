...
async def _ensure_media_downloaded(media_id: int) -> Media:
    async with SessionLocal() as session, session.begin():
        ...
        checksum, size = await download_telegram_file(
            bot, media.telegram_file_id, storage_key, settings
        )
...
async def _deliver_protected_media(job: Job) -> None:
    ...
    async with SessionLocal() as session:
        ...
        caption = (
            ...
            "Копия сохранена после вашего ответа на защищённое сообщение."
        )
        ...
        if media.media_type == "photo":
            ...
            await bot.send_video(user.telegram_id, video=file, caption=caption, protect_content=True)
        elif media.media_type == "voice":
            await bot.send_voice(user.telegram_id, voice=file, caption=caption, protect_content=True)
        elif media.media_type == "video_note":
            await bot.send_video_note(user.telegram_id, video_note=file, protect_content=True)
            await bot.send_message(user.telegram_id, caption)
        elif media.media_type == "audio":
            await bot.send_audio(user.telegram_id, audio=file, caption=caption, protect_content=True)
...
async def recover_stale_running_jobs() -> int:
    ...
    async with SessionLocal() as session, session.begin():
        result = await session.execute(
            ...
            .values(status="queued", locked_at=None, available_at=datetime.now(UTC))
        )
