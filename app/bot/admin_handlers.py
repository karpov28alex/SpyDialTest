...
from app.db.models import BusinessConnection, FailedUpdate, Job, Message as DbMessage, Payment, User
...
async def stats_text() -> str:
    ...
    new_users = await _safe_scalar(select(func.count(User.id)).where(User.registered_at >= since))
    ...
    edited = await _safe_scalar(select(func.count(DbMessage.id)).where(DbMessage.edited_at.is_not(None)))
    deleted = await _safe_scalar(select(func.count(DbMessage.id)).where(DbMessage.is_deleted.is_(True)))
    errors = await _safe_scalar(
        select(func.count(FailedUpdate.id)).where(FailedUpdate.resolved.is_(False))
    )
    queued = await _safe_scalar(select(func.count(Job.id)).where(Job.status == "queued"))
    return (
        ...
        f"📦 Заданий в очереди: <b>{queued}</b>\n"
...
async def admins_text() -> str:
    ...
    try:
        ...
        await redis.aclose()
    dynamic = sorted(int(value) for value in values if str(value).isdigit())
    ...
    lines.extend(f"👮 <code>{item}</code> — добавленный" for item in dynamic if item not in configured and item != OWNER_ADMIN_ID)
    lines.append("\nДобавить: <code>/admin_add TELEGRAM_ID</code>")
    lines.append("Удалить: <code>/admin_remove TELEGRAM_ID</code>")
...
async def admin_remove_command(message: Message, command: CommandObject) -> None:
    ...
    if target_id in settings.telegram_admin_ids:
        await message.answer("Этот администратор задан в .env. Удалите его из TELEGRAM_ADMIN_IDS и перезапустите сервис.")
        return
    redis = await _redis()
    try:
...
async def admin_callback(callback: CallbackQuery) -> None:
    ...
    try:
        if action == "stats":
            ...
            async with SessionLocal() as session:
                users = list((await session.scalars(select(User).order_by(User.id.desc()).limit(20))).all())
            text = "<b>👥 Последние пользователи</b>\n\n" + (
                "\n".join(f"<code>{u.telegram_id}</code> · @{u.username or '—'}" for u in users)
                if users else "Пользователей нет."
            )
        elif action == "revenue":
            ...
            text = f"<b>💰 Доход</b>\n\nПолучено за всё время: <b>{Decimal(total):,.2f} ₽</b>"
        elif action == "errors":
            async with SessionLocal() as session:
                errors = list((await session.scalars(
                    select(FailedUpdate).where(FailedUpdate.resolved.is_(False)).order_by(FailedUpdate.id.desc()).limit(5)
                )).all())
            text = "<b>⚠️ Последние ошибки</b>\n\n" + (
                "\n\n".join(f"#{row.id} · {row.update_type}\n<code>{row.error[:250]}</code>" for row in errors)
                if errors else "Необработанных ошибок нет."
            )
        else:
            text = (
                "<b>📨 Рассылки</b>\n\n"
