...
EDITABLE_FIELDS = {
    ...
    "referral_text": "Текст приглашения друга",
...
def expired_keyboard(payment_url: str, payment_button_text: str) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(text="👥 Пригласить друга", callback_data="funnel:invite")]]
...
def extended_admin_keyboard() -> InlineKeyboardMarkup:
    ...
    insert_at = max(len(rows) - 1, 0)
    rows.insert(insert_at, [InlineKeyboardButton(text="🔐 Доступ и воронка", callback_data="funnel:admin")])
    return keyboard(rows)
...
async def send_access_screen(message: Message, user) -> None:
    ...
    async with SessionLocal() as session:
        ...
        state = await access_state(session, db_user or user)

    if config.enabled and not state.active:
        text = config.referral_text if config.referral_required and not user.referral_bonus_granted_at else config.payment_required_text
        await message.answer(text, reply_markup=expired_keyboard(config.payment_url, config.payment_button_text))
        return
...
async def start(message: Message, command: CommandObject) -> None:
    ...
    async with SessionLocal() as session, session.begin():
        user, _ = await register_or_update_user(
            session,
...
async def admin_command(message: Message) -> None:
    ...
    if not await is_admin(user_id):
        ...
        return
    await message.answer(
        "<b>🛡 Phantom — центр управления</b>\n\nВсе основные действия доступны по кнопкам.",
...
async def fields(callback: CallbackQuery) -> None:
    ...
    groups = {
        ...
        "texts": ["subscription_text", "subscription_error_text", "subscription_success_text", "referral_text"],
...
async def edit_field(callback: CallbackQuery) -> None:
    ...
    if callback.message:
        await callback.message.answer(
            f"Отправьте новое значение для поля «{EDITABLE_FIELDS[field]}».\n\nДля отмены отправьте /cancel."
        )
    await callback.answer()
