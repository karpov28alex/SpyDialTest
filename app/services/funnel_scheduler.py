    ...
    *,
    ...
    payment = Payment(
        ...
        payload={
            ...
            "impaya_user_id": source_payload.get("impaya_user_id"),
            "merchant_user_id": source_payload.get("merchant_user_id", ""),
    ...
    try:
        response = await client.recurrent_pay(
            ...
            description=f"Продление VIP-доступа Phantom на {access_days} дней",
        )
        state = await client.transaction_state(
            customer_operation_id=operation_id,
            extended=True,
            terminal_name=settings.impaya_non3ds_terminal_name,
        )
...
async def process_impaya_renewals() -> int:
    ...
    async with SessionLocal() as session:
        ...
        for user in users:
            ...
            primary = await _renewal_attempt(
                session,
                user=user,
                source=source,
                operation_id=f"ph_renew_{user.id}_{period_key}",
                amount_rub=settings.impaya_renewal_amount_rub,
                access_days=settings.impaya_renewal_access_days,
                kind="renewal",
            )
            result = primary
            if primary.status != "paid" and settings.impaya_fallback_enabled:
                result = await _renewal_attempt(
                    session,
                    user=user,
                    source=source,
                    operation_id=f"ph_fallback_{user.id}_{period_key}",
                    amount_rub=settings.impaya_fallback_amount_rub,
                    access_days=settings.impaya_fallback_access_days,
                    kind="fallback",
                )
            ...
            try:
                if result.status == "paid":
                    ...
                    await bot.send_message(
                        user.telegram_id,
                        "<b>⚠️ Не удалось продлить подписку</b>\n\n"
