...
async function api(path,opt={}){
  const r=await fetch(path,{...opt,headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',...(opt.headers||{})},cache:'no-store'});
  if(!r.ok){const body=await r.json().catch(()=>null);throw new Error(body?.error?.message||body?.detail||`HTTP ${r.status}`)}
  return r.json();
}
...
function scheduleRefresh(){
  stopRefresh();
  if(state.screen!=='dialogs'||document.hidden||mediaPlaying)return;
  refreshTimer=setTimeout(()=>render(true),5000);
}

function route(screen,id=null,push=true){
  stopRefresh();
  state={screen,id};
  firstDialogRender=true;
...
function avatar(x,size=''){const fallback=esc((x.peer_name||x.peer_username||'?')[0].toUpperCase());return x.avatar?`<span class="avatar ${size}"><img src="${esc(x.avatar)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove();this.parentElement.textContent='${fallback}'"></span>`:`<span class="avatar ${size}">${fallback}</span>`}
...
async function profile(){
  ...
  return top('Профиль','Настройки сохраняются без перезагрузки')+`<main class="page"><section class="profile-card"><div class="profile-head"><span class="avatar">${esc((me.first_name||'D')[0])}</span><div><div class="profile-name">${esc([me.first_name,me.last_name].filter(Boolean).join(' ')||'Пользователь')}</div><div class="muted">${me.username?'@'+esc(me.username):'Telegram ID '+esc(me.telegram_id)}</div></div></div><div class="row"><span>Telegram Business</span><b>${me.business_connected?'Подключён':'Не подключён'}</b></div><div class="row"><span>Статус доступа</span><b>${esc(me.subscription_status)}</b></div><div class="row"><span>Доступ до</span><b>${fmt(me.access_ends_at)}</b></div><div class="row"><span>В сервисе с</span><b>${fmt(me.registered_at)}</b></div></section><section class="settings-card"><h3>Основные функции</h3>${settingRow('save_protected_media','Сохранять скрытые медиа','При ответе сохранить одноразовое медиа в архиве и диалоге',s.save_protected_media)}${settingRow('notifications_enabled','Получать уведомления','Главный выключатель сообщений от бота',notificationsOn)}<div class="settings-nested">${settingRow('notify_edits','Изменения сообщений','Показывать предыдущую и новую версии',s.notify_edits,!notificationsOn)}${settingRow('notify_deletions','Удалённые сообщения','Присылать сохранённую копию',s.notify_deletions,!notificationsOn)}${settingRow('notify_protected_media','Копия защищённого медиа','Отправлять сохранённый файл в бот',s.notify_protected_media,!notificationsOn)}</div></section><section class="settings-card"><h3>Приватность и оформление</h3>${settingRow('hide_preview','Скрывать текст уведомлений','Показывать только тип события и кнопку Mini App',s.hide_preview)}${settingRow('notify_emoji','Эмодзи в уведомлениях','Использовать визуальные маркеры событий',s.notify_emoji)}<div class="row"><div><b>Тема интерфейса</b><div class="muted">Выберите оформление</div></div><div class="theme-buttons"><button class="theme-btn ${s.theme==='dark'?'active':''}" data-theme="dark">Тёмная</button><button class="theme-btn ${s.theme==='light'?'active':''}" data-theme="light">Светлая</button></div></div></section></main>`;
}
...
function syncNotificationDependants(enabled){
  for(const key of ['notify_edits','notify_deletions','notify_protected_media']){
...
function bindMediaState(){
  document.querySelectorAll('audio,video').forEach(el=>{
    el.addEventListener('play',()=>{mediaPlaying=true;stopRefresh()});
    const done=()=>{mediaPlaying=false;scheduleRefresh()};
    el.addEventListener('pause',done);el.addEventListener('ended',done);
  });
}
...
async function render(silent=false){
  ...
  if(!silent)app.innerHTML='<div class="boot"><div class="spinner"></div></div>';
  try{
    const html=state.screen==='home'?home():state.screen==='dialogs'?await dialogs():state.screen==='dialog'?await dialog():state.screen==='profile'?await profile():home();
    ...
    if(seq!==renderSeq)return;
    app.innerHTML=`${state.screen!=='home'?top('Ошибка загрузки'):''}<main class="page"><div class="error">${esc(e.message||'Не удалось загрузить данные')}<br><button class="retry" data-retry>Повторить</button><button class="retry" data-go="home">На главную</button></div></main>`;
  }
}
...
app.addEventListener('click',async e=>{
  ...
  if(th){
    ...
    document.querySelectorAll('[data-theme]').forEach(x=>x.classList.remove('active'));th.classList.add('active');
    const previous=settings?.theme;
    applyTheme(th.dataset.theme);
    try{const r=await api('/api/settings',{method:'PATCH',body:JSON.stringify({theme:th.dataset.theme})});settings=r.settings}
    catch(err){applyTheme(previous);tg?.showAlert?.(err.message||'Не удалось сохранить тему')}
  }
});

app.addEventListener('change',async e=>{
  const key=e.target.dataset.setting;if(!key)return;
  const input=e.target;const previous=!input.checked;
  input.disabled=true;
  if(key==='notifications_enabled')syncNotificationDependants(input.checked);
  try{
    const result=await api('/api/settings',{method:'PATCH',body:JSON.stringify({[key]:input.checked})});
    settings=result.settings;
    input.checked=Boolean(settings[key]);
    if(key==='notifications_enabled')syncNotificationDependants(Boolean(settings.notifications_enabled));
  }catch(err){
    input.checked=previous;
    if(key==='notifications_enabled')syncNotificationDependants(previous);
    tg?.showAlert?.(err.message||'Не удалось сохранить настройку');
...
(async()=>{
  initTelegram();
  if(!tg?.initData){app.innerHTML='<main class="page"><div class="error">Откройте приложение через Telegram-бота</div></main>';return}
  try{
    const r=await fetch('/api/auth/telegram',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({init_data:tg.initData}),cache:'no-store'});
    if(!r.ok)throw new Error('Ошибка безопасного входа');
    token=(await r.json()).access_token;
    const u=new URL(location.href);const requested=u.searchParams.get('screen');
    state={screen:['home','dialogs','dialog','profile'].includes(requested)?requested:'home',id:u.searchParams.get('id')};
