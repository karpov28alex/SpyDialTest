...
async def dashboard(_: AdminAuth, session: Session) -> dict:
    ...
    revenue_today = await session.scalar(select(func.coalesce(func.sum(Payment.amount), 0)).where(Payment.status == "paid", Payment.paid_at >= today))
    revenue_month = await session.scalar(select(func.coalesce(func.sum(Payment.amount), 0)).where(Payment.status == "paid", Payment.paid_at >= month))
    revenue_total = await session.scalar(select(func.coalesce(func.sum(Payment.amount), 0)).where(Payment.status == "paid"))
    metrics = {
    ...
    registration_rows = (await session.execute(
        ...
        .group_by(func.date(User.registered_at)).order_by(func.date(User.registered_at))
    )).all()
    event_rows = (await session.execute(
        select(
            func.date(Message.created_at).label("day"), func.count(Message.id),
    ...
    recent_users = list((await session.scalars(select(User).order_by(desc(User.registered_at)).limit(8))).all())
    return {
        "metrics": metrics,
        "registrations": [{"date": str(day), "count": count} for day, count in registration_rows],
        "events": [{"date": str(day), "messages": total, "edited": edited, "deleted": deleted} for day, total, edited, deleted in event_rows],
        "recent_users": [serialize_user(user) for user in recent_users],
    }


@router.get("/users")
async def users(_: AdminAuth, session: Session, search: str = "", limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict:
    query = select(User)
    if search.strip():
        term = f"%{search.strip()}%"
...
async def user_detail(user_id: int, _: AdminAuth, session: Session) -> dict:
    ...
    messages_count = int(await session.scalar(select(func.count(Message.id)).join(Dialog, Dialog.id == Message.dialog_id).where(Dialog.owner_user_id == user.id)) or 0)
    ...
    recent_messages = list((await session.scalars(
        select(Message).join(Dialog, Dialog.id == Message.dialog_id)
        .where(Dialog.owner_user_id == user.id, Message.created_at >= since)
        .order_by(desc(Message.created_at)).limit(60)
    )).all())
    activity = []
    for item in recent_messages:
        ...
        activity.append({"type": kind, "at": item.created_at.isoformat(), "message_id": item.id})
    for connection in connections:
        if connection.last_activity_at and connection.last_activity_at.replace(tzinfo=None) >= since:
            activity.append({"type": "Активность Telegram Business", "at": connection.last_activity_at.isoformat(), "connection_id": connection.id})
    activity.sort(key=lambda row: row["at"], reverse=True)
    return {
        **serialize_user(user),
        "dialogs_count": dialogs_count,
        "messages_count": messages_count,
        "registration_source": f"Реферальная ссылка {referral.code}" if referral else "Прямой запуск или поиск в Telegram",
        "referrer": serialize_user(referrer) if referrer else None,
        "activity_24h": activity[:60],
        "connections": [{
            "id": row.id, "active": row.is_active,
...
async def user_dialogs(user_id: int, _: AdminAuth, session: Session) -> dict:
    ...
    items = []
    for row in rows:
        count = int(await session.scalar(select(func.count(Message.id)).where(Message.dialog_id == row.id)) or 0)
        items.append({
            "id": row.id, "name": row.peer_name, "username": row.peer_username,
            "telegram_chat_id": row.telegram_chat_id, "avatar": row.avatar,
            "last_message_at": row.last_message_at.isoformat() if row.last_message_at else None,
            "messages_count": count, "is_hidden": row.is_hidden,
        })
    return {"items": items}


@router.get("/dialogs/{dialog_id}/messages")
async def dialog_messages(dialog_id: int, _: AdminAuth, session: Session, settings: Settings = Depends(get_settings), limit: int = Query(200, ge=1, le=500)) -> dict:
    dialog = await session.get(Dialog, dialog_id)
    if not dialog:
        raise HTTPException(status_code=404, detail="Dialog not found")
    rows = list((await session.scalars(select(Message).where(Message.dialog_id == dialog_id).order_by(Message.sent_at).limit(limit))).all())
    result = []
    for message in rows:
        ...
        result.append({
            "id": message.id, "telegram_message_id": message.telegram_message_id,
            "direction": message.direction, "text": message.text, "caption": message.caption,
            "sent_at": message.sent_at.isoformat(), "edited_at": message.edited_at.isoformat() if message.edited_at else None,
            "deleted_at": message.deleted_at.isoformat() if message.deleted_at else None, "is_deleted": message.is_deleted,
            ...
            "media": [{
                "id": m.id, "type": m.media_type, "protected": m.is_protected,
                "status": m.download_status, "filename": m.filename, "size": m.size,
                "url": media_url(m, settings),
            } for m in media],
        })
    return {"dialog": {"id": dialog.id, "name": dialog.peer_name, "username": dialog.peer_username, "avatar": dialog.avatar}, "items": result}


@router.get("/protected-media")
async def protected_media(_: AdminAuth, session: Session, settings: Settings = Depends(get_settings), limit: int = Query(100, ge=1, le=500)) -> dict:
    rows = (await session.execute(
        select(Media, Message, Dialog).join(Message, Message.id == Media.message_id).join(Dialog, Dialog.id == Message.dialog_id)
        .where(Media.is_protected.is_(True)).order_by(desc(Media.created_at)).limit(limit)
    )).all()
    return {"items": [{
        "id": media.id, "type": media.media_type, "filename": media.filename,
        "size": media.size, "status": media.download_status, "created_at": media.created_at.isoformat(),
        "dialog_id": dialog.id, "dialog_name": dialog.peer_name, "message_id": message.id,
        "url": media_url(media, settings),
    } for media, message, dialog in rows]}


@router.get("/media/download/{token}", include_in_schema=False)
async def admin_media_download(token: str, session: Session, settings: Settings = Depends(get_settings)):
    try:
