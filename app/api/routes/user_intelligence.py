...
from app.api.routes.user import require_channel_access
...
async def _export_rows(session: SessionDep, user_id: int):
    ...
    if message_ids:
        media_rows = list(
            (await session.scalars(select(Media).where(Media.message_id.in_(message_ids)))).all()
        )
...
def _build_export(format_name: str, user, rows, media_by_message) -> tuple[bytes, str, str]:
    ...
    if format_name == "csv":
        ...
        return ("\ufeff" + stream.getvalue()).encode("utf-8"), filename, "text/csv"
...
h1{{color:#c88cff}}h3{{margin:0 0 4px}}.meta,small{{color:#ad9dbc}}p{{white-space:pre-wrap}}
    ...
    return document.encode("utf-8"), filename, "text/html"


async def _prepare_export(format_name: str, user: CurrentUser, session: SessionDep):
    await require_channel_access(user)
    ...
    return _build_export(format_name, user, rows, media_by_message)


@router.get("/export/{format_name}")
async def export_archive(format_name: str, user: CurrentUser, session: SessionDep):
    payload, filename, media_type = await _prepare_export(format_name, user, session)
...
@router.post("/export/{format_name}/send")
async def send_export_to_bot(format_name: str, user: CurrentUser, session: SessionDep) -> dict:
    payload, filename, _ = await _prepare_export(format_name, user, session)
    await bot.send_document(
        chat_id=user.telegram_id,
        document=BufferedInputFile(payload, filename=filename),
        caption=(
            "📦 <b>Ваш архив Phantom готов</b>\n\n"
            f"Формат: <b>{format_name.upper()}</b>\n"
            "Файл отправлен документом и доступен для сохранения или пересылки."
        ),
    )
