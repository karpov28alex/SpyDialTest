"""Create global monetization settings after notification controls.
...
def upgrade() -> None:
    ...
    if "app_monetization_settings" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.Column(
                "payment_placeholder_url",
                sa.String(length=1024),
                nullable=False,
                server_default="https://game.hidenow.su/app?screen=subscription&demo=1",
            ),
...
def downgrade() -> None:
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "app_monetization_settings" in inspector.get_table_names():
